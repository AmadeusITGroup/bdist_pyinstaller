...
def get_pip_index_url():
    ...
    if sys.version_info >= (3, 5):
        ...
        raise NotImplementedError("This extension is meant to be used with Python 3.5 or newer")
    return ret_val
...
class PyInstalerCmd(Command):
    ...
    """
    description = 'create pyinstaller bundle using the console entry points as dispatch points'
    ...
    def run(self):
        ...
        subprocess.check_call(
            ' '.join(chain([sys.executable, "-m", "pip", "install", "."], index_url_args)), shell=True)

        subprocess.check_call(' '.join(
            chain((sys.executable, "-m", "pip", "install", "pyinstaller", "psutil", "ipython"), \
                index_url_args)), shell=True)
        ...
        for console_script in self.distribution.entry_points.get('console_scripts'):
            ...
            if m:
                console_scripts.add((m.groupdict().get('script_name'), m.groupdict().get(
                    'package_name'), m.groupdict().get('function_name')))
        ...
        sample_import_module = ''
        if list(chain(package_imports, function_imports)):
            sample_import_module = list(chain(package_imports, function_imports))[
                0][0].split('.')[0]
...
""")
            # imports
            pyinstaller_dispatcher_fl.write(
                '\n'.join(['import {}'.format(p) for p in package_imports]))
            pyinstaller_dispatcher_fl.write(
                '\n'.join(['from {} import {} as {}'.format(p, f, fqn_name(p, f)) for p, f in function_imports]))
...
def main():
    ...
    try:
        ...
        return 2
    return 0
            """.format(package_name=self.distribution.get_name(), sample_import_module=sample_import_module))
...
""".format(script_name=script_name, package_name=package_name, function_name=fqn_name(package_name, function_name)))
                else:
                    pyinstaller_dispatcher_fl.write("""
...
""".format(script_name=script_name, package_name=package_name, function_name=function_name))
...
""")
        ...
        try:
            ...
            packages_to_harvest = set([p.split('.', 1)[0] for p in self.distribution.packages])
            packages_to_harvest.add('parso')  # Note: It is required for IPython

            for package_name in packages_to_harvest:
                ...
                for root, dirs, files in os.walk(PACKAGE__ROOT):
                    for _file_ in files:
                        ...
                        _python_module_path_segments_ = os.path.join(
                            _package_.__name__, src[len(PACKAGE__ROOT)+1:]).split(os.sep)
                        if _python_module_path_segments_[-1] == '__init__':
                            _python_module_ = '.'.join(_python_module_path_segments_[:-1])
                        else:
                        ...
                        dst = os.path.join('./', _package_.__name__,
                                           src[len(PACKAGE__ROOT)+1:])
                        extra_data.add((src,  os.path.dirname(dst)))

            add_extras_cmd = []
            [add_extras_cmd.extend(['--add-binary', '{}:{}'.format(item[0], item[1])])
             for item in extra_binaries]
            [add_extras_cmd.extend(['--add-data', '{}:{}'.format(item[0], item[1])])
             for item in extra_data]
            [add_extras_cmd.extend(['--hidden-import', '{}'.format(item)])
             for item in hidden_imports]

            pyinstaller_dist = self.distribution.command_options.get('bdist_pyinstaller', {})\
                .get('dist_dir', ('',os.path.join(os.getcwd(), 'pyinstaller_dist')))[1]
            if not os.path.exists(pyinstaller_dist):
                os.makedirs(pyinstaller_dist)

            one_dir = self.distribution.command_options.get('bdist_pyinstaller', {}).get('one_dir', False)
            extra_args = self.distribution.command_options.get('bdist_pyinstaller', {}).get('extra_args', ('',''))[1]
            if extra_args:
                '''
            ...
            target_name = '{}-{}'.format(self.distribution.get_name(), self.distribution.get_version())
            ...
            pyinstaller_run()
            if one_dir:
                with tarfile.open(os.path.join(pyinstaller_dist, '{}.tar.gz'.format(target_name)), \
                    mode='w:gz') as archive:
                    archive.add(name=os.path.join(pyinstaller_dist, target_name), arcname=target_name, recursive=True)
        finally:
            sys.argv = _argv_
